    ...
    size_t input_length, uint8_t *output, size_t output_size)
{
    ...
    const size_t required_output =
        direction ==
        MBEDTLS_PSA_ASYNC_CRYPTO_ENCRYPT ? payload_length + tag_length : payload_length;
...
complete_signature:
    if (operation->stage == MBEDTLS_PSA_ASYNC_CRYPTO_SIGNATURE_RANDOM) {
        ...
        if (!mbedtls_psa_async_crypto_valid_nonce(operation->nonce, operation->curve)) {
            ...
            status = mbedtls_psa_async_crypto_start(operation->provider,
        ...
        operation->request.nonce_scalar =
            (mbedtls_psa_async_crypto_const_buffer_t){ operation->nonce,
                                                       operation->curve->order.length };
...
{
    if (operation != NULL) {
        mbedtls_psa_async_crypto_zeroize(operation->shared_secret,
                                         sizeof(operation->shared_secret));
        memset(operation, 0, sizeof(*operation));
    }
}
