                               ...
                               mbedtls_pk_context *pk)
{
    ...
    if (pk_alg == MBEDTLS_PK_RSA) {
        ret = pk_parse_rsa_pubkey(pk, *p, (size_t) (end - *p));
        if (ret == 0) {
    ...
    const unsigned char *key, size_t keylen)
{
    ...
    if (pk_alg == MBEDTLS_PK_RSA) {
        if ((ret = pk_parse_rsa_key(pk, p, len)) != 0) {
            mbedtls_pk_free(pk);
            return ret;
        }
    } else
                         ...
                         const unsigned char *pwd, size_t pwdlen)
{
    ...
    if (ret == 0) {
        ...
        if ((ret = mbedtls_pk_setup(pk, pk_info)) != 0 ||
            (ret = pk_parse_rsa_key(pk, pem.buf, pem.buflen)) != 0) {
            mbedtls_pk_free(pk);
        }
    ...
    if (mbedtls_pk_setup(pk, pk_info) == 0 &&
        pk_parse_rsa_key(pk, key, keylen) == 0) {
        return 0;
    }
                                ...
                                const unsigned char *key, size_t keylen)
{
    ...
    if (ret == 0) {
        ...
        if ((ret = pk_parse_rsa_pubkey(ctx, p, pem.buflen)) != 0) {
            mbedtls_pk_free(ctx);
        }
    ...
    p = (unsigned char *) key;
    ret = pk_parse_rsa_pubkey(ctx, p, keylen);
    if (ret == 0) {
        return ret;
    }
    mbedtls_pk_free(ctx);
