    ...
    uint8_t *data, size_t data_size, size_t *data_length)
{
    ...
    int ret = PQCP_MLDSA_NATIVE_MLDSA87_keypair_internal(data,
                                                         secret,
                                                         key_buffer);
    if (ret != 0) {
    ...
    uint8_t *signature, size_t signature_size, size_t *signature_length)
{
    *signature_length = 0;      /* Safe default */
    ...
    int ret = PQCP_MLDSA_NATIVE_MLDSA87_keypair_internal(public,
                                                         secret,
                                                         key_buffer);
    if (ret != 0) {
        /* The only documented error is an internal error which is not
    ...
    ret = PQCP_MLDSA_NATIVE_MLDSA87_signature_internal(signature,
                                                       signature_length,
    ...
    uint8_t *signature, size_t signature_length)
{
    ...
    int ret = PQCP_MLDSA_NATIVE_MLDSA87_verify(signature, signature_length,
                                               message, message_length,
                                               NULL, 0,
                                               key_buffer);
    if (ret == 0) {
