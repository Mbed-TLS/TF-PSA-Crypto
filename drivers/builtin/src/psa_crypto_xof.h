 ...
 *         The size of the \p xof buffer is too small. You can determine a
 *         sufficient buffer size by calling #PSA_XOF_LENGTH(\c alg)
 *         where \c alg is the xof algorithm that is calculated.
 * \retval #PSA_ERROR_INSUFFICIENT_MEMORY \emptydescription
 * \retval #PSA_ERROR_CORRUPTION_DETECTED \emptydescription
 */
